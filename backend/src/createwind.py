...
# Ensure the packaged code is importable when running from the backend folder
...
def main():
    ...
    # Resolve paths relative to backend root (this script now lives in backend/cli)
    repo_root = BACKEND_ROOT.parent
    
    tangent_map_path = Path(args.tangent_map)
    if not tangent_map_path.is_absolute():
        # Prefer backend-relative paths, then repo-root paths
        backend_candidate = BACKEND_ROOT / args.tangent_map
        repo_candidate = repo_root / args.tangent_map
        if backend_candidate.exists():
            tangent_map_path = backend_candidate
        else:
            tangent_map_path = repo_candidate
    tangent_map_path = tangent_map_path.resolve()
    if not tangent_map_path.exists():
        print(f"Error: Tangent map not found at '{tangent_map_path}'. Provide a valid path with --tangent-map.")
        sys.exit(2)
    
    output_dir = BACKEND_ROOT / 'var' / 'output'
    output_dir.mkdir(parents=True, exist_ok=True)
    # Keep a hook for other modules that expect the default path attribute
    setattr(config, "DEFAULT_OUTPUT_DIR", str(output_dir))
    
    # Step 1: Load and preprocess the tangent map data
    valid_points, all_grad_vectors, all_positions, col_labels = data_processing.preprocess_tangent_map(str(tangent_map_path))
    ...
    grid_data = grid_computation.build_grids(
        all_positions, grid_res, grad_indices, all_grad_vectors, col_labels, output_dir=str(output_dir)
    )
    ...
    visualization_core.save_final_figure(fig, str(output_dir), "featurewind_modular_figure.png")
