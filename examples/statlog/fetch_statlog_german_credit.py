#!/usr/bin/env python3
"""
Fetch the UCI Statlog (German Credit Data) dataset (ID=144) using ucimlrepo,
print basic info, and write a CSV with numeric 'label' + normalized features.

Usage:
  python examples/statlog/fetch_statlog_german_credit.py
...
def main():
    ...
    # Normalize/clean column names to strings then replace AttributeN with meaningful names
    ...
    X_df = X_df.rename(columns=name_map)
    ...
    out_path = os.path.join(script_dir, 'statlog_german_credit.csv')
...
def make_continuous_and_normalize(df):
    ...
    for col in out.columns:
        ...
        if pd.api.types.is_numeric_dtype(s):
            ...
            # Map categoricals to [0,1] by ordinal codes with NaNs handled
