#!/usr/bin/env python3
"""
Fetch and prepare the UCI Connectionist Bench (Sonar, Mines vs. Rocks) dataset (id=151).

Output: a CSV with a leading `label` column (target class) and all other
normalized feature columns scaled to [0, 1] (min-max).

Usage:
  python examples/sonar/generate_sonar.py
  python examples/sonar/generate_sonar.py --out examples/sonar/sonar_normalized.csv

Notes:
- Requires `ucimlrepo` package. Install with: pip install ucimlrepo
- Only numeric features are normalized. Non-numeric targets are kept as-is in `label`.
"""
...
def main():
    ap = argparse.ArgumentParser(description='Fetch and normalize UCI Sonar (id=151) dataset')
    ap.add_argument('-o', '--out', type=str, default='', help='Output CSV path (default: examples/sonar/sonar_normalized.csv)')
    args = ap.parse_args()
    ...
    X = sonar.data.features  # pandas DataFrame (numeric)
    y = sonar.data.targets   # pandas DataFrame (class labels e.g., 'M'/'R')
    ...
    if y is None or y.shape[1] == 0:
        ...
        if y.shape[1] == 1:
            ...
            # If multiple targets, take the first as label
    ...
    # Output path
    out_dir = Path('examples/sonar')
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = Path(args.out) if args.out else out_dir / 'sonar_normalized.csv'

    out_df.to_csv(out_path, index=False)
    print(f'Saved normalized Sonar dataset to: {out_path}')
