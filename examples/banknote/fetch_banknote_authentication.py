#!/usr/bin/env python3
"""
Fetch the UCI Banknote Authentication dataset (ID=267) using ucimlrepo,
print basic info, and write a CSV with numeric 'label' + normalized features.

Usage:
  python examples/banknote/fetch_banknote_authentication.py
...
def main():
    ...
    # fetch dataset (UCI ID 267: Banknote Authentication)
    ds = fetch_ucirepo(id=267)
    ...
    # Numeric label column 'label' (handles string labels if any)
    label_series = to_numeric_label(y_series)

    # Normalize features to [0,1] (numeric: min-max; categorical: ordinal codes normalized)
    ...
    out_path = os.path.join(script_dir, 'banknote_authentication.csv')
    out_df.to_csv(out_path, index=False)

    # Print summary + metadata/variables
